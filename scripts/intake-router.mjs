...
// WorkBuddy 依赖（静默降级：非 WorkBuddy 宿主时使用空实现）
let adaptIntakeProtocol, extractUserProfile;
try {
  ({ adaptIntakeProtocol, extractUserProfile } = await import('./workbuddy-user-profile-bridge.mjs'));
} catch {
  adaptIntakeProtocol = (profile) => profile || null;
  extractUserProfile = () => null;
}
...
let generateSessionSnapshot;
try {
  ({ generateSessionSnapshot } = await import('./workbuddy-session-snapshot.mjs'));
} catch {
  generateSessionSnapshot = async () => {};
}
import { applyBookMemoryTemplate } from './apply-book-memory-template.mjs';
let runRetroToSkillCandidates;
try {
  ({ runRetroToSkillCandidates } = await import('./retro-to-skill-candidates.mjs'));
} catch {
  runRetroToSkillCandidates = () => [];
}

// 通用画像提取（非 WorkBuddy 宿主用）
let extractOpenClawProfile;
try {
  ({ extractOpenClawProfile } = await import('./lib/openclaw-profile-bridge.mjs'));
} catch {
  extractOpenClawProfile = () => null;
}
...
function buildProfileContext(hostCap) {
  // 尝试多个位置的用户画像文件，按优先级返回第一个有完整画像的
  const candidateDirs = [];

  // 1. WorkBuddy 原生目录
  const wbHome = hostCap?.workbuddy?.homeDir;
  if (wbHome && hostCap?.hostType === 'workbuddy') candidateDirs.push(wbHome);

  // 2. OpenClaw 工作区（USER.md / IDENTITY.md / SOUL.md）
  const ocWorkspace = path.join(os.homedir(), '.openclaw', 'workspace');
  if (fs.existsSync(ocWorkspace)) candidateDirs.push(ocWorkspace);

  // 3. 通用画像目录（用户可自行创建）
  const fbsProfile = path.join(os.homedir(), '.fbs-profile');
  if (fs.existsSync(fbsProfile)) candidateDirs.push(fbsProfile);

  for (const dir of candidateDirs) {
    try {
      // OpenClaw 工作区使用专用解析器（模板格式不同）
      const isOC = dir.includes('.openclaw');
      const profile = isOC ? extractOpenClawProfile(dir) : extractUserProfile(dir);
      if (profile && (profile.isProfileComplete || profile.basicInfo.name || profile.workContext.currentProject)) {
...
function readResumeCard(resolvedFbsDir) {
  ...
  const OLD = path.join(resolvedFbsDir, 'workbuddy-resume.json');
  const NEW = path.join(resolvedFbsDir, 'session-resume.json');
  try { if (fs.existsSync(OLD) && !fs.existsSync(NEW)) fs.renameSync(OLD, NEW); } catch {}

  const resumePath = path.join(resolvedFbsDir, 'session-resume.json');
...
function collectNestedBookRoots(startDir, maxDepth, maxNodes, maxResults = 12) {
  ...
  while (queue.length && nodes < maxNodes && hits.length < maxResults) {
    ...
    if (fs.existsSync(path.join(fbs, 'chapter-status.md')) || fs.existsSync(path.join(fbs, 'session-resume.json'))) {
      hits.push(dir);
    }
    if (depth >= maxDepth) continue;
    let entries;
...
function scoreProjectAnchorCandidate(bookRoot) {
  ...
  push(fs.existsSync(path.join(fbsDir, 'session-resume.json')), 3, '存在恢复卡');
...
function collectProjectHintsFromLocalMemory(resolvedBookRoot) {
  const files = [
    path.join(resolvedBookRoot, '.fbs', 'session-resume.json'),
...
function collectExternalDriveFbsRoots(resolvedBookRoot, maxResults = 8) {
  ...
  for (const driveRoot of listWindowsDriveRoots()) {
    ...
    for (const d1 of lv1) {
      ...
      if (fs.existsSync(path.join(p1, '.fbs', 'chapter-status.md')) || fs.existsSync(path.join(p1, '.fbs', 'session-resume.json'))) {
      ...
      for (const d2 of lv2) {
        ...
        if (fs.existsSync(path.join(p2, '.fbs', 'chapter-status.md')) || fs.existsSync(path.join(p2, '.fbs', 'session-resume.json'))) {
...
function hasMemoryBriefSource(effectiveBookRoot, effectiveFbsDir, memoryProvider) {
  ...
  return [
    ...
    path.join(effectiveFbsDir, 'session-resume.json'),
...
async function ensureRuntimeArtifacts(resolvedBookRoot, effectiveBookRoot, effectiveFbsDir, memoryProvider) {
  ...
  const resumeJson = path.join(effectiveFbsDir, 'session-resume.json');
  ...
  if (!fs.existsSync(resumeJson) && (fs.existsSync(chapterStatus) || fs.existsSync(esmState))) {
    try {
      ...
      if (!resumeSnapshotRegenerated) artifactErrors.push('恢复卡补写后仍未生成 session-resume.json');
    } catch (error) {
...
function buildTermValueGlossary() {
  return [
    { term: '.fbs/session-resume.json', role: '会话恢复卡', userValue: '下次继续时不必重讲背景，直接从上次断点开工。' },
...
export function buildRecommendations(intent, env) {
  ...
  if (env.resumeSnapshotRegenerated) {
    info.push('已自动补写 .fbs/session-resume.json');
  }
    ...
    case 'resume': {
      if (!env.hasResumeJson && env.hasChapterStatus) {
        blockers.push({
          ...
          message: '缺少标准恢复卡 `.fbs/session-resume.json`，且自动补写未成功；续写前必须先生成恢复卡快照。',
        });
        actions.push({
          step: actions.length + 1,
          label: '生成恢复卡快照',
          cmd: `node scripts/workbuddy-session-snapshot.mjs --fbs-dir "${env.fbsDir}" --quiet`,
          reason: '缺少 `.fbs/session-resume.json`，需先生成标准恢复卡再进入续写',
          required: true,
        });
      }

      const resumeFile = env.hasResumeJson
        ? path.join(env.fbsDir, 'session-resume.json')
...
if (process.argv[1] && process.argv[1].endsWith('intake-router.mjs')) {
  const { tryMain } = await import("./lib/user-errors.mjs");
  tryMain(main, { friendlyName: "入口路由", jsonOutput: false });
}
